---
title: Contact
name: Contact
meta_title: Contact MPL Box Fit | Haslingden Boxing Studio
meta_description: >-
  Contact MPL Box Fit in Haslingden about classes, junior boxing, private
  group training and 1-to-1 boxing coaching.
permalink: /contact/
eleventyNavigation:
  key: Contact
  order: 5
blocks:
  - type: image-background
    image: /images/hex-dumbbell-rack-new.jpg
    image_alt: Training session underway on the MPL Box Fit floor
    parallax: true
    tint: true
    content: |
      # Contact Us

      Tell us what you are after - classes, junior boxing, private group
      training, strength work or 1-to-1 coaching - and we will get back to
      you. If you are new to it, just say so, and we will point you at the
      right session.

  - type: features
    intro_content: |
      ## How to Reach Us
    items:
      - icon: "mdi:map-marker"
        name: Where We Are
        description: "Inside Monkey Playland, Haslingden, Rossendale."
      - icon: "mdi:calendar-clock"
        name: Class Times
        description: >-
          Ladies: Mon & Fri 10–11am and Mon & Wed 6–7pm. Men: Tue & Thu
          7–8pm. £10 a session.
      - icon: "mdi:email"
        name: Email Us
        description: >-
          Email [info@mplboxfit.co.uk](mailto:info@mplboxfit.co.uk), message
          us on Facebook or Instagram, or fill in the form below and we will
          get back to you.

  - type: contact-form
    intro_content: |
      ## Send Us a Message
    content: |
      Fill in the form and we will get back to you. If it helps, let us know
      whether the training is for you, your child or a private group, and
      how much experience you have, so we can suggest the right session.

  - type: icon-links
    intro_content: |
      ## Find Us Online
    items:
      - icon: "hugeicons:facebook-01"
        text: "Follow us on Facebook"
        url: "https://www.facebook.com/profile.php?id=61576716490057"
      - icon: "hugeicons:instagram"
        text: "Follow @mpl.boxfit on Instagram"
        url: "https://www.instagram.com/mpl.boxfit"
      - icon: "hugeicons:linkedin-01"
        text: "Connect with Coach Umair on LinkedIn"
        url: "https://www.linkedin.com/in/umair-khan-pm"
---
